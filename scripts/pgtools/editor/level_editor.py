...
from scripts.pgtools.tilemap import Tilemap
from scripts.pgtools.font import Font
...
class LevelEditor:
    ...
    def autotile_rect(self): # keybinding: t
        if self.selection_rect: # only with rect
            tilemap = self.tilemap.tilemap
            for loc in tilemap:
                for layer in (int(layer) for layer in tilemap[loc]):
                    layer = str(layer)
                    if self.current_layer == layer:
                        layer = str(layer)
                        tile = tilemap[loc][layer]
                        neighbours = []
                        for offset in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                            tile_loc = (tile['tile_pos'][0] + offset[0], tile['tile_pos'][1] + offset[1])
                            if not self.selection_rect.collidepoint((tile_loc[0] * self.tile_size, tile_loc[1] * self.tile_size)):  # only with rect
                                continue  
                            str_loc = str(tile_loc[0]) + ';' + str(tile_loc[1])
                            if str_loc in tilemap:
                                if layer in tilemap[str_loc]:
                                    if tile['type'] == tilemap[str_loc][layer]['type'] and tile['type'] in self.tilemap.autotile_types:
    ...
    def autotile_layer(self): # keybinding: shift + t
        tilemap = self.tilemap.tilemap
        for loc in tilemap:
            for layer in (int(layer) for layer in tilemap[loc]):
                layer = str(layer)
    ...
    def floodfill(self, curr_pos, tile_data, offset=(0, 0)):
        ...
        while floodfill_list:            
            ...
            for b in bordering_tiles:
                if not self.tilemap.get_tile_by_layer(b, tile_data['layer']) and tuple(b) not in visited:
    ...
    def remove_all_tiles(self): # keybinding: x
        if self.selection_rect: # only with rect
            ...
            for layer in self.tilemap.offgrid_tiles:
                tile_list = self.tilemap.offgrid_tiles[layer]
                for tile_data in tile_list:
                    if self.selection_rect.collidepoint((tile_data['pos'])):  # only with rect
                        self.tilemap.offgrid_tiles[layer].remove(tile_data)
    ...
    def run(self):
        while True:
            ...
            self.font.render(self.display, file_text, (self.display.get_width() - self.font.get_width(file_text) - 5, 7))
            
            placement_text = 'placement_mode: ' + str(self.placement_mode)
            self.font.render(self.display, placement_text, (self.display.get_width() - self.font.get_width(placement_text) - 5, 23))
            
            layer_text = 'layer: ' + str(self.current_layer)
            self.font.render(self.display, layer_text, (self.display.get_width() - self.font.get_width(layer_text) - 5, 38))
            
            pos_text = 'pos: ' + str(list(scaled_mpos))
            self.font.render(self.display, pos_text, (self.display.get_width() - self.font.get_width(pos_text) - 5, 53))
            
            tile_pos_text = 'tile_pos: ' + str(list(tile_pos))
            self.font.render(self.display, tile_pos_text, (self.display.get_width() - self.font.get_width(tile_pos_text) - 5, 68))
            
            tilemap_size_text = 'tilemap size: ' + str(len(self.tilemap.tilemap) + sum([len(self.tilemap.offgrid_tiles[tile]) for tile in self.tilemap.offgrid_tiles]))
            self.font.render(self.display, tilemap_size_text, (self.display.get_width() - self.font.get_width(tilemap_size_text) - 5, 83))
                        
            # sidebar scrolling
            if mpos[0] < self.sidebar_size:
                if mpos[1] < 100: # top bar
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_t: # autotile with the rectangle
                        if self.shift:
                            self.autotile_layer()
                        else:
                            self.autotile_rect()
                    ...
                    if event.key == pygame.K_f:
                        tile_data = {'type': self.tile_list[self.tile_group], 'variant': self.tile_variant, 'layer': self.current_layer}
                        self.floodfill(tile_pos, tile_data, self.scroll)
                        self.reset_rect()
